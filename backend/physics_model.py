...
def calculate_damage_radii(diameter_m, density_kg_m3, velocity_kms, angle_deg, target_type):
    ...
    # --- 1. Calculate Impact Energy (no changes in this section) ---
    ...
    # --- 2. Calculate Crater Diameter (no changes in this section) ---
    ...
    # --- 3. Calculate Thermal Radiation Radius (no changes in this section) ---
    thermal_radius_km = 0.8 * (energy_megatons ** 0.41)

    # --- 4. Calculate Air Blast Radius (no changes in this section) ---
    blast_radius_km = 3.0 * (energy_megatons ** (1/3))

    # --- 5. Calculate Seismic Shaking Radius (no changes in this section) ---
    ...
    # --- MODIFICATION: Return both radii and energy ---
    return {
        "radii": {
...
if __name__ == "__main__":
    ...
    chelyabinsk_results = calculate_damage_radii(
        diameter_m=20, density_kg_m3=3300, velocity_kms=19, angle_deg=20, target_type="sedimentary_rock"
    )
    print(chelyabinsk_results)

    print("\nScenario 2: Significant Threat (150m diameter)")
    significant_results = calculate_damage_radii(
        diameter_m=150, density_kg_m3=3000, velocity_kms=25, angle_deg=45, target_type="sedimentary_rock"
    )
